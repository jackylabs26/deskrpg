...
특정 릴리스를 고정하고 싶다면 `.env.docker`의 `DESKRPG_IMAGE`를 `dandacompany/deskrpg:0.1.6`처럼 바꾸면 됩니다.
...
특정 이미지 버전을 쓰고 싶다면 명령 앞에 `DESKRPG_IMAGE=dandacompany/deskrpg:0.1.6`을 붙이면 됩니다.
