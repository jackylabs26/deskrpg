...
Before tagging `0.2.2`, verify:

1. `package.json` version is `0.2.2`
...
```bash
JWT_SECRET=change-me DESKRPG_IMAGE=dandacompany/deskrpg:0.2.2 \
...
3. Bump the version
4. Push a new tag such as `0.2.2`
5. If needed, pin deployments with `DESKRPG_IMAGE=dandacompany/deskrpg:<known-good-tag>`

## Notes

- The release tag format is currently `0.2.2`, not `v0.2.2`
